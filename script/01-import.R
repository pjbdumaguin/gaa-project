...
base_page <- "https://www.dbm.gov.ph/index.php/"

# Doesn't work. Some fiscal years have a slightly different link.
gaa_pages <- paste0(base_page, seq(2020, 2024),
...
# This works.
gaa_pages <- get_links(page = paste0(base_page, "budget"),
                       pattern = "\\/general\\-appropriations")

dl_links <- map_chr(gaa_pages, get_links(.x, "csv|xlsx?") |> first())
